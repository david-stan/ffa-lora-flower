...
def formatting_prompts_func(example):
    ...
    # gsm8k prompt
    for i in range(len(example["instruction"])):
        text = f"Q: {example['instruction'][i]} A: {example['response'][i]}"
        output_texts.append(text)

    return output_texts

def format_gsm8k_prompt(example):
    # Assume the dataset has keys "question", "explanation", and "final_answer"
    prompt = (
        f"Q: {example['response']}\n"
        f"A: {example['instruction']}"
    )
    return {"text": prompt}
...
def get_tokenizer_and_data_collator_and_propt_formatting(model_name: str):
    ...
    response_template_with_context = "#### " # gsm8k
